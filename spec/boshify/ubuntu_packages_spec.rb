  ...
  describe UbuntuPackages do
    describe 'mirror url' do
      ...
      it 'allows the mirror to be specified' do
        pkg_source = UbuntuPackages.new(downloader: double, cmd_runner: double, mirror_url: 'http://uk.archive.ubuntu.com/ubuntu')
...
Files:
 ...
 7b2315bdb243d9d63260f72fec0bebc8 34003 postgresql-8.4_8.4.3-1.diff.gz'
    ...
    it 'refreshes package metadata' do
      downloader = double
      cmd_runner = double
      expect(downloader).to receive(:get).with(URI.parse('http://us.archive.ubuntu.com/ubuntu/dists/lucid/main/source/Sources.bz2'))
      expect(cmd_runner).to receive(:run).with(/bzcat/).and_return(postgresql_package)
      pkg_source = UbuntuPackages.new(downloader: downloader, cmd_runner: cmd_runner)
      pkg_source.refresh
    end
    it 'accepts mirror urls with trailing slashes' do
      downloader = double
      cmd_runner = double
      expect(downloader).to receive(:get).with(URI.parse('http://uk.archive.ubuntu.com/ubuntu/dists/lucid/main/source/Sources.bz2'))
      expect(cmd_runner).to receive(:run).with(/bzcat/).and_return(postgresql_package)
      pkg_source = UbuntuPackages.new(downloader: downloader, cmd_runner: cmd_runner, mirror_url: 'http://uk.archive.ubuntu.com/ubuntu/')
      pkg_source.refresh
    end
    it 'determines the source tarball url for a given package' do
      downloader = double
      cmd_runner = double
      allow(downloader).to receive(:get).with(URI.parse('http://us.archive.ubuntu.com/ubuntu/dists/lucid/main/source/Sources.bz2'))
      allow(cmd_runner).to receive(:run).with(/bzcat/).and_return(postgresql_package)
      pkg_source = UbuntuPackages.new(downloader: downloader, cmd_runner: cmd_runner)
      pkg_source.refresh
      expect(pkg_source.source_tarball_url('postgresql-8.4')).to eq(URI.parse('http://us.archive.ubuntu.com/ubuntu/pool/main/p/postgresql-8.4/postgresql-8.4_8.4.3.orig.tar.gz'))
    end
    it 'raises if the requested package is not in the index' do
      ...
      end.to raise_error(PackageNotFoundError, 'Package postgres-8.4 was not found')
    end
...
Version: 1.0.5-4'
      expect(UbuntuPackages.new.parse(input)).to eq([
        { 'Package' => 'bzip2', 'Binary' => 'libbz2-1.0, libbz2-dev, bzip2, lib64bz2-1.0, lib64bz2-dev, lib32bz2-1.0, lib32bz2-dev, bzip2-doc', 'Version' => '1.0.5-4' }
      ])
    end
    it 'parses package properties' do
      input = 'Package: bzip2
...
Files:
 ...
 49559e20e5bb230a6b1a4221f08ddbb5 76602 bzip2_1.0.5-4.diff.gz'
      expect(UbuntuPackages.new.parse(input)[0]['Files']).to eq([
        { name: 'bzip2_1.0.5-4.dsc', size_bytes: 1423, checksum: 'adcf07cad22f7ba3cde606fc49162e93' },
        { name: 'bzip2_1.0.5.orig.tar.gz', size_bytes: 841_402, checksum: '3c15a0c8d1d3ee1c46a1634d00617b1a' },
        { name: 'bzip2_1.0.5-4.diff.gz', size_bytes: 76_602, checksum: '49559e20e5bb230a6b1a4221f08ddbb5' }
      ])
    end
