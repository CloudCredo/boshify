...
# Convert packages to BOSH releases
module Boshify
  describe PackageConverter do
    ...
    subject do
      PackageConverter.new(package_source: pkg_source,
    ...
    it 'refreshes the package source metadata' do
      ...
      subject.create_release_for(name: 'postgresql-8.4')
    end

    it 'downloads the source tarball for the package' do
      allow(pkg_source).to receive(:source_tarball_url).and_return(
        'http://example.org/code.tar.gz')
    ...
    it 'creates a release from the discovered source tarball' do
      allow(pkg_source).to receive(:source_tarball_url).and_return(
        'http://example.org/code.tar.gz')
      allow(downloader).to receive(:get).with(
        'http://example.org/code.tar.gz').and_return('/local/path/code.tar.gz')
      expect(release_creator).to receive(:create_release).with(
        name: 'postgresql-8.4',
        packages: [
          name: 'postgresql-8.4',
